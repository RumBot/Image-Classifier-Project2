...
# ------------------------------------------------------------------------------- #
# arg_parser() parses arguments from the command line
def arg_parser():
    # Define a parser
    ...
    # Specify top-k
    parser.add_argument(
        "--top_k", type=int, help="Choose top K matches as int."
    )
...
# load_checkpoint(checkpoint_path) loads our saved deep learning model from checkpoint
...
# process_image(image_path) performs cropping, scaling of image for our model
def process_image(image_path):
    ...
    np_image = (
        np.array(test_image) / 255
    )  # Divided by 255 because imshow() expects integers (0:1)!!
...
def predict(image_tensor, model, device, cat_to_name, top_k):
    ...
    torch_image = torch.from_numpy(np.expand_dims(image_tensor, axis=0)).type(
        torch.FloatTensor
    )
    ...
    # I borrowed this part since I also had GPU/CPU issues
...
def main():
    ...
    top_probs, top_labels, top_flowers = predict(
        image_tensor, model, device, cat_to_name, args.top_k
    )
